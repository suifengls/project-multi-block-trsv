...
int main()
{
    ...
    printf("Start on GPU...\n");
    trsvOnDevice(b, x, cval, cind, ccnt, size, si, level, lind);
    ...
    if(DEBUG)
    {
        ...
        for(int i = size*size-20; i < size*size; i++)
        {
            printf("%10.6lf\t", L[i]);
            if(i%size==size-1)
                printf("\n");
...
void trsvOnDevice(float *b, float *x, float *val, int *index, int *count, int size, int si, int *level, int *lind)
{
    ...
    dim3 dimGrid(1, 1);
    //int th = 1024 < size? 1024: size;
    dim3 dimBlock(size, 1);
    trsvKernel<<<dimGrid, dimBlock>>>(bd, xd, vald, indexd, countd, leveld, lindd, nl);
